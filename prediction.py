...
from assets import column_names, mode_data, months, neighbourhood
...
def make_prediction(model_path):
    ...
    # for i in range(6):
    #     input_X[column_names[i]] = [st_columns[i].selectbox(f'{column_names[i]}', value = mode_data[column_names[i]])]
    ...
    input_X[column_names[7]] = [st_columns[7].text_input(f'{column_names[7]}', value = reported_date.month, disabled = True)]
    ...
    input_X[column_names[13]] = [st_columns[13].text_input(f'{column_names[13]}', value = occurence_date.month, disabled = True)]
